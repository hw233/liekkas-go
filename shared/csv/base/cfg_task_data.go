...
type CfgTaskDataConfig struct {
	data map[int32]*CfgTaskData
}

func NewCfgTaskDataConfig() *CfgTaskDataConfig {
	return &CfgTaskDataConfig{
		data: make(map[int32]*CfgTaskData),
	}
}

func (c *CfgTaskDataConfig) Load(filePath string) bool {
	parse := NewParser()
	...
	for i := 2; i < parse.GetAllCount(); i++ {
		...
		if !IdRet {
			glog.Error("Parse CfgTaskData.Id field error,value:", vId)
			return false
		}
		...
		if !ModuleRet {
			glog.Error("Parse CfgTaskData.Module field error,value:", vModule)
			return false
		}
		...
		if !ConditionTypeRet {
			glog.Error("Parse CfgTaskData.ConditionType field error,value:", vConditionType)
			return false
		}
		...
		if vecConditionParams != "" {
			...
			for j := 0; j < len(arrayConditionParams); j++ {
				...
				if !ret {
					glog.Error("Parse CfgTaskData.ConditionParams field error, value:", arrayConditionParams[j])
					return false
				}
		...
		if !TargetCountRet {
			glog.Error("Parse CfgTaskData.TargetCount field error,value:", vTargetCount)
			return false
		}
		...
		if !IsAutoReceiveRet {
			glog.Error("Parse CfgTaskData.IsAutoReceive field error,value:", vIsAutoReceive)
			return false
		}
		...
		if !AddPointRet {
			glog.Error("Parse CfgTaskData.AddPoint field error,value:", vAddPoint)
			return false
		}
		...
		if !DropIdRet {
			glog.Error("Parse CfgTaskData.DropId field error,value:", vDropId)
			return false
		}
		...
		if !GroupRet {
			glog.Error("Parse CfgTaskData.Group field error,value:", vGroup)
			return false
		}
...
func (c *CfgTaskDataConfig) Clear() {
}

func (c *CfgTaskDataConfig) Find(id int32) (*CfgTaskData, bool) {
	v, ok := c.data[id]
	return v, ok
}

func (c *CfgTaskDataConfig) GetAllData() map[int32]*CfgTaskData {
	return c.data
}

func (c *CfgTaskDataConfig) Traverse() {
